...
export class ChatComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  @Input() showAllAgents: boolean = false; // New property to show all agents' messages
  ...
  ngOnInit(): void {
    ...
    if (this.agentId) {
      // If we're showing all agents, use the getAllAgentsMessages method
      if (this.showAllAgents) {
        const allMessagesSub = this.chatService.getAllAgentsMessages().subscribe(messages => {
          this.messages = messages;
          this.shouldScroll = true;
        });
        this.subscriptions.push(allMessagesSub);
      } else {
        ...
        // Load chat history for the current agent only
