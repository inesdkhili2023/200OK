...
export class NotificationService implements OnDestroy {
  ...
  markAllAsRead(): void {
    ...
    // For each unread notification, mark it as read on the server
    currentNotifications
      .filter(n => !n.read)
      .forEach(n => {
        this.http.post(`${this.apiUrl}/${n.id}/read`, {}).subscribe({
          error: (error) => {
            console.error('Error marking notification as read:', error);
            this.hasBackendError = true;
          }
    ...
    this.http.get<any[]>(this.apiUrl)
      .pipe(
        ...
        catchError(error => {
          ...
          const mockNotifications: TowingNotification[] = this.generateMockNotifications();
          this._notifications.next(mockNotifications);
          return of(mockNotifications);
        })
      ).subscribe();
